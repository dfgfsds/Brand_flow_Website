...
export default function CartSummary({ totalAmount }: any) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      setError(error?.response?.data?.error?.error || "Failed to apply coupon. Please try again.");
    } finally {
      setIsChecking(false);
    }
  };
    ...
    <>
      {data?.data?.length ? (
        <div className="space-y-3 mb-3">
          <div className="flex items-center justify-between">
            ...
            <Link
              ...
              className="text-sm text-blue-900 font-bold  hover:text-blue-700"
        ...
        <div className="text-center p-4 bg-gray-50 rounded-lg mb-3">
          ...
          <p className="mt-2 inline-block text-sm text-blue-900 hover:text-blue-700 font-bold cursor-pointer"
            onClick={() => {
      ...
      <div className="bg-[#F8F7F2] rounded-xl p-3">
                    ...
                    ?.map((coupon: any) => (
                      ...
                      >
                        <div className="flex justify-between">
                          <span className="font-bold text-blue-900 capitalize">{coupon?.code}</span>
        ...
        <div className="mt-6 space-y-4">
          ...
          ${paymentValue === item?.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'}`}
              >
                <input
                  ...
                  className="mt-1 h-4 w-4 text-blue-900 border-gray-300 focus:ring-red-700"
                />
                <div className="ml-3">
          ...
          {!data?.data?.length ? (
            <button className="w-full p-2 bg-blue-900 cursor-pointer hover:bg-red-700 text-white font-bold"
              onClick={() => {
                router.push('/profile?tab=addresses')
              }}
            >
            ...
            <>
              <button className="w-full p-2 flex gap-2 justify-center bg-blue-900 cursor-pointer hover:bg-red-700 text-white font-bold"
